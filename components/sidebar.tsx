...
import { Home, Calendar, Package, File, Settings, LogOut } from 'lucide-react'
  ...
  return (
    <>
      <div className="hidden md:flex flex-col fixed left-0 top-0 h-screen w-60 bg-card-bg backdrop-blur-lg border-r border-gray-200 pt-8 px-6">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-foreground">{mockProvider.businessName}</h1>
          <p className="text-text-secondary text-sm mt-1">{mockProvider.serviceCategory}</p>
        </div>
        ...
        <Button variant="ghost" className="w-full justify-start px-4 py-3 text-foreground hover:bg-gray-100">
          <LogOut size={20} />
